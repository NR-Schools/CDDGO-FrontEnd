...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['reserve'])) {
        ...
        $interval = $currentDate->diff($selectedDate);
        $daysDifference = $interval->d;
        if ($daysDifference < 0) {
            echo "<script>
                alert('Invalid Date for Reservation');
        ...
        if (!$result) {
            // Reservation Failed!
            echo <<<EOD
                <script> alert('Reservation Failed: {$error}');
                ...
                <div class="date-container">
                    ...
                    <div>
                        <input class="date-input" name="date" type="date"
                            min="<?php echo date('Y-m-d', strtotime('+1 day', strtotime('tomorrow'))); ?>">
                    </div>
                </div>
                <div class="divider"></div>
